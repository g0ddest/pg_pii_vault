...
use crate::{PII_VAULT_URL, PII_VAULT_TOKEN, PII_VAULT_MOUNT};
...
pub fn get_key_from_vault(key_id: &[u8]) -> Result<[u8; 32], String> {
    ...
    let url = url_guc.to_str().map_err(|e: std::str::Utf8Error| e.to_string())?;
    let token = token_guc.to_str().map_err(|e: std::str::Utf8Error| e.to_string())?;
    let mount_str;
    ...
    let export_resp: VaultExportResponse = resp.json().map_err(|e| format!("Failed to parse Vault response: {}", e))?;
    
    // Transit export returns keys in a map, version as key
    let latest_key_base64 = export_resp.data.keys.values().next().ok_or("No key found in Vault response")?;
    let key_bytes = general_purpose::STANDARD.decode(latest_key_base64).map_err(|e| format!("Failed to decode key: {}", e))?;
...
fn create_key_in_vault(url: &str, token: &str, mount: &str, key_name: &str) -> Result<(), String> {
    ...
    if !resp.status().is_success() {
         return Err(format!("Vault create key returned error: {}", resp.status()));
    }
    Ok(())
}
